...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const isScroll = window.scrollY > 260;
        ...
        <Container className='d-flex align-items-center justify-content-between'>
          ...
          <nav className={`heading-menu ${isMenu ?'show-menu':''}`}>
            <div className="title d-flex align-items-center justify-content-between d-lg-none">
              ...
              <button className="heading-toggler me-md-4" onClick={() => setIsMenu(!isMenu)}>
            ...
            <ul className="d-lg-flex align-items-center">
              {navLinks.map((link, index) => (
                <li key={index} className='d-block'>
                  <Link spy={true} smooth={true} duration={700} className="heading-link text-capitalize" to={link.to} onTouchStart={() => handleTouchStart(link.to)}>{link.btn_title}</Link>
                </li>
              ))}
